...
const lockSeats = asyncHandler(async (req, res) => {
    ...
    // 1. CLEANUP: If user already has a pending booking for this showtime, release those seats first
    const oldPending = await Booking.findOne({ userId, showtimeId, status: 'Pending' });
    if (oldPending) {
        showtime.pendingSeats = showtime.pendingSeats.filter(seat => !oldPending.seatsBooked.includes(seat));
        await Booking.findByIdAndDelete(oldPending._id);
    }

    // 2. Lock NEW seats for 10 minutes
    const expiry = new Date(Date.now() + 10 * 60 * 1000);
    showtime.pendingSeats.push(...seatsToLock);
    await movie.save();

    // 3. Create fresh pending booking
...
const confirmBooking = asyncHandler(async (req, res) => {
    ...
    if (booking.bookingExpiry < new Date()) {
        ...
        await releaseSeatsHelper(booking.movieId, booking.showtimeId, booking.seatsBooked);
    ...
    // Generate Multiple QR codes (One per seat)
    const qrCodes = [];
    for (const seat of booking.seatsBooked) {
        const qr = await generateQRCode(booking._id, { ...bookingDetails, seats: [seat] });
        qrCodes.push({ seat, url: qr });
    }
    booking.qrCodes = qrCodes;
    if (qrCodes.length > 0) booking.qrCodeUrl = qrCodes[0].url; // Primary QR for compatibility
    await booking.save();
    // Send email
...
// @desc    Delete booking from history
...
module.exports = { createBooking, getUserBookings, lockSeats, confirmBooking, releaseSeats, cancelBooking, getWalletBalance, addToWallet, deleteBooking };